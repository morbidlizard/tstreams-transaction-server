...
| bootstrap.port                                    | Port to bind.  |int    |8071| |
| bootstrap.open-ops-pool-size                      | Size of the ordered pool that contains single thread executors to work with transaction open operations. |int | Runtime.getRuntime.availableProcessors() |positive integer |
...
| authentication.key                                | The key to authorize.  |string |key| |
| authentication.key-cache-size                     | The number of active tokens a server can handle over time.  |int    |10000| [1,...]|
| authentication.key-cache-expiration-time-sec      | The time a token live before expiration.  |int    | 600| [1,...]|
| storage-model.file-prefix                         | The path where folders of Commit log, berkeley environment and rocksdb databases would be placed.  |string |/tmp| |
...
| rocksdb.max-background-compactions                | Is the maximum number of concurrent background compactions. The default is 1, but to fully utilize your CPU and storage you might want to increase this to approximately number of cores in the system.  |int    | 1| [1,...]|
