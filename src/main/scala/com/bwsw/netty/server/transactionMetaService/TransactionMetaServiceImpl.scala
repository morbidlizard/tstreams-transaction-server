package com.bwsw.netty.server.transactionMetaService
...
//  }
  ...
  private def doesProducerTransactionExpired(txn: com.bwsw.netty.server.transactionMetaService.ProducerTransaction): Boolean =
    (txn.keepAliveTTL + config.transactionMetadataTtlAdd) <= Instant.now().getEpochSecond
