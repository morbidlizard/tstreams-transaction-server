...
object ServerOptions {
  ...
  /** The options are applied on creation Rocksdb database.
    ...
    * @param transactionDatabaseTransactionKeeptimeMin   the time a producer transaction live after if it transits to one of final state: Checkpointed, Invalid.(default: 6 months). If negative integer - transaction lives forever.
