...
class ServerClientInterconnectionLifecycleTest extends FlatSpec with Matchers with BeforeAndAfterEach {
  ...
  it should "put stream, then put producerTransaction with states in following order: Opened->Checkpointed. Should return Checkpointed Transaction" in {
    ...
    val producerTransaction = com.bwsw.tstreamstransactionserver.rpc.ProducerTransaction(
      stream.name, stream.partitions, System.currentTimeMillis(), TransactionStates.Opened, -1, openedTTL
    )
  ...
  it should "put stream, then put producerTransaction with states in following order: Opened->Updated->Updated->Updated->Checkpointed. Should return Checkpointed Transaction" in {
    ...
    transitOneTransactionToAnotherState(transactionServiceServer, producerTxnOpened, producerTxnUpdated1, producerTxnOpened, openedTTL - TimeUnit.SECONDS.toMillis(2))
    ...
    transitOneTransactionToAnotherState(transactionServiceServer, producerTxnOpened, producerTxnUpdated2, producerTxnOpened, updatedTTL2 - TimeUnit.SECONDS.toMillis(2))
    ...
    transitOneTransactionToAnotherState(transactionServiceServer, producerTxnOpened, producerTxnUpdated3, producerTxnOpened, updatedTTL3 - TimeUnit.SECONDS.toMillis(2))
    ...
    transitOneTransactionToAnotherState(transactionServiceServer, producerTxnOpened, producerTransactionCheckpointed, producerTransactionCheckpointed, checkpointedTTL - TimeUnit.SECONDS.toMillis(2))
